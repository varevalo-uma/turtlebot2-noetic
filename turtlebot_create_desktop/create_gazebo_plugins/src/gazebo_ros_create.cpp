...
void GazeboRosCreate::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf )
{
  ...
  prev_update_time_ = last_cmd_vel_time_ = this->my_world_->SimTime(); // varevalo SimTime instead of GetSimTime
  odom_pose_[0] = 0.0;
  odom_pose_[1] = 0.0;
  odom_pose_[2] = 0.0;
...
{
  common::Time time_now = this->my_world_->SimTime();  // varevalo SimTime instead of GetSimTime
  ...
  {
    js_.position[0] = joints_[LEFT]->Position(0); // varevalo Position(0) instead of GetAngle(0).Radian()
    js_.velocity[0] = joints_[LEFT]->GetVelocity(0);
  }

  if (this->set_joints_[RIGHT])
  {
    js_.position[1] = joints_[RIGHT]->Position(0); // varevalo Position(0) instead of GetAngle(0).Radian()
    js_.velocity[1] = joints_[RIGHT]->GetVelocity(0);
  }

  if (this->set_joints_[FRONT])
  {
    js_.position[2] = joints_[FRONT]->Position(0); // varevalo Position(0) instead of GetAngle(0).Radian()
    js_.velocity[2] = joints_[FRONT]->GetVelocity(0);
  }

  if (this->set_joints_[REAR])
  {
    js_.position[3] = joints_[REAR]->Position(0); // varevalo Position(0) instead of GetAngle(0).Radian()
...
{
  last_cmd_vel_time_ = this->my_world_->SimTime(); // varevalo SimTime instead of GetSimTime
  double vr, va;
  vr = msg->linear.x;
  va = msg->angular.z;
